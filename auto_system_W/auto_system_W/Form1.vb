...
Public Class Form1
    ...
    Private Sub RegistrationUpdate()
        Dim response As Integer
            ...
            If (profile_id.SelectedValue = Nothing) Then
                ...
                If response = vbYes Then
                    ...
                    Else
                        ...
                        connection.ConnectionString = connstring
                        connection.Open()
                        query = "SELECT * FROM users_tab WHERE phone_number=@phone_number"
                        command = New MySqlCommand(query, connection)
                        command.Parameters.AddWithValue("@phone_number", phoneno_txt.Text)
                        reader = command.ExecuteReader
                        If reader.HasRows = True Then
    ...
    Private Sub Registration()
        ...
        Catch ex As Exception
            ...
            response = MessageBox.Show("Image Size is too Bigger, Kindly Reduce the Size to Continue!", "Auto System Application", MessageBoxButtons.OK, MessageBoxIcon.Error)
